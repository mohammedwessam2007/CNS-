...
| v18.2 | Pushed 26 Sep 2026 to the same branch (sessions zero, timbuktu and names). **What is confirmed, through the Vercel connector (read only):** the project now lists its deployments. Production is `dpl_FH791VyN3rgbK7ky1TxmUGXF2mCn`, **READY**, built from the v18.1 checkpoint `2c577a0` (its title reads v18.0). The v18.1 commit `23f58f0` produced **no deployment** in the listing, for reasons the connector does not show. The connector still cannot read a deployment's detail (404) or fetch its pages. After the v18.2 push (`e177767`) **no deployment was created** either. The project made 12 deployments that day (limit 100), so the cause is not the quota; the connector shows neither the Git settings nor a reason. A production deployment was not forced (`receipts/v18_2/RECEIPT.md`). |
| v18.0–v18.1 | Pushed 26 Sep 2026 to the same branch (Renaissance engine v18, season 3, the sealed battery, then the genome, media registry, trials L4–L11, the hosting session, the requirement graph and coverage oracle). **Not confirmed live from the build session**: the Vercel connector sees the team but not this project (list deployments 403, deployment by URL 404, fetch: no access), the sandbox blocks `*.vercel.app`, and the GitHub connector has no commit-status tool. The identical build (`deploy/vercel/build.mjs`) passed every suite on :8790 (`receipts/v18_1/full_regression.log`). To confirm: open the live URL; the tab title should read v18.1 |
