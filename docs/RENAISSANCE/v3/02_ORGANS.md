# v3 · 02 · The organs, the cultivation tests, global coverage, and the frontier

§16–34, §216, §226, §230–235 of the ABSOLUTE COMPLETION mission. Every organ below is a real, playable experience in
the app (test V6 completes each one; V4 runs them on a phone), not a named empty component (§218). Depth per organ is
one complete experience; breadth is the perpetual frontier (last section), counted by the coverage oracle as open.
...
| Film (§23) | cut | moves a camera round two actors and sees when screen direction flips; learns the rule and Ozu's deliberate breaks | V6 |
| Architecture (§24) | arch | hangs a chain, flips it into an arch, loads it, compares a half-circle; reads New Gourna's mud-brick vaults | V6 |
...
| Test | Instrument in the app | What it measures | Status |
|---|---|---|---|
| **Salon** (§30) | the salon session; the sealed weekly unknown problems (a field no session taught) | contributing precisely, asking the question that opens the other person, moving between fields | instrument built; the real test is a real dinner (`socialfeedback` tap, §232) |
| **Museum** (§31) | `RENAISSANCE.perception().museum`: cold predictions against later answers on new works, for the art sessions, and the same ideas days later | whether he sees more than before (structure, technique), not whether he knows labels | instrument built (test L7); results need his answers |
| **Concert** (§32) | `RENAISSANCE.perception().concert`, the same for the music session, with synthesised listening | whether he hears structure, tension, resolution and style | instrument built (L7) |
| **Conversation** (§33, §226) | t13 (continue a real disagreement), k9 (a misquotation at dinner), the counterfeit learners that reward trivia, pretension and fake certainty with nothing | no trivia flexing, no fake certainty, no quote dumping, no pretentious language | built (V6, V11, S3) |

Rates in every instrument are withheld below five answers (no fake precision, §166).

## 3. Global coverage (§26, §173) — honest depth

| Region | Sessions set there | Appears as a case in |
|---|---|---|
...
The machinery for scale exists and is tested: the civilisation graph, the masterpiece compiler, the possession
ladder, the compiler that picks unpossessed works. What does not exist yet is volume: thousands of works, every form
(drama, epic, essay, memoir, satire, myth, religious text), every medium (painting and sculpture need lawful images;
recorded performance needs licensed audio). §169 says there is no terminal syllabus, so this requirement is never
counted as closed; the next works queued for the compiler, one per form: *Hamlet* (drama), the *Odyssey* in a
public-domain translation (epic), Montaigne's "Of Cannibals" (essay), Ibn Battuta's travels (memoir), *Candide*
(satire), the *Epic of Gilgamesh* (myth), and the Qur'anic story of Joseph with its commentaries (religious text,
treated with the seriousness §25 asks for).
