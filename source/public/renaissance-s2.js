...
(function () {
  ...
  sessions.push({
    ...
    steps: [
      ...
      { id: "n4", type: "model", stage: "model", min: 3, model: "pipeline", src: ["little"], terms: ["little"], title: "Move the capacities", body: "25 patients arrive each hour. Change any step and watch the flow and the queue.", ask: "**Find two things:** a change that does nothing, and the smallest change that raises the flow. Then read the waiting time: it comes from Little's law." },
      {
        ...
        q: { stem: "What do both managers get wrong, and what would help?", options: [{ t: "Both improved a step with spare capacity; only the scanner and grill set the pace", ok: true, why: "So only more scanning or more grilling raises the flow. Different worlds, same three boxes." }, { t: "Both should buy the most expensive equipment for every single step", bug: "surface", why: "Price has nothing to do with which box constrains." }, { t: "Both should make every step equally fast so nothing ever waits", bug: "notconstraint", why: "That wastes money on steps that were never limiting." }, { t: "Airports and kitchens can't be compared: their customers want different things", bug: "surface", why: "The surfaces differ; the structure is identical." }] },
      },
      { id: "n6", type: "q", kind: "transfer", stage: "transfer", min: 2, atoms: ["minimal", "orient"], stem: "A team ships features slowly. Designers finish 6 designs a week, engineers build 5, one senior reviewer approves 2, testers check 8. Everyone argues about meetings and tools. What are the three boxes and the constraint?", options: [{ t: "Design → build → review → test; the reviewer, at 2 a week", ok: true, why: "The reviewer's 2 a week limits everything downstream. Two features a week is the ceiling until review capacity grows (a second reviewer, or smaller pieces to review)." }, { t: "Meetings → tools → morale; morale is the constraint", bug: "omission", why: "Those may matter, but they are not steps the work flows through; start with the flow." }, { t: "Engineers at 5 a week, because they are the most expensive people", bug: "notconstraint", why: "Cost isn't capacity; 5 a week is more than the reviewer's 2." }, { t: "Testers at 8 a week, because testing always slows releases", bug: "surface", why: "They have the most spare capacity of all." }] },
      { id: "n7", type: "q", kind: "far", stage: "far", min: 1.5, src: ["little"], atoms: ["compress", "prob"], stem: "A café has, on average, 10 customers inside, and 20 come in each hour. How long does a typical customer stay?", options: [{ t: "30 minutes", ok: true, why: "Little's law: people inside = arrivals per hour × time each stays → 10 = 20 × W → W = half an hour. It works for cafés, hospitals, inboxes and queues of patients." }, { t: "2 hours", bug: "inversion", why: "That divides the wrong way round (20 ÷ 10)." }, { t: "10 minutes", bug: "surface", why: "The numbers don't support it." }, { t: "Impossible to know without timing people", bug: "omission", why: "The law gives it from two averages, no stopwatch needed." }] },
      {
        ...
        critique: { stem: "A week later the pile has moved to a different box. What does that mean?", options: [{ t: "Success: the constraint moved; draw the boxes again", ok: true, why: "Every improvement of the constraint creates a new one." }, { t: "Failure: the change didn't work, so go back to the old plan", bug: "consistency", why: "A moving constraint is the expected result of fixing one." }, { t: "Work harder on every box so that no pile can form anywhere", bug: "notconstraint", why: "Effort on non-constraints is the original mistake." }] },
      },
    ],
    challenge: { stem: "A hospital lab processes 200 samples a day, but the only analyser handles 120. The hospital hires more sample collectors. What happens to results per day?", options: [{ t: "Nothing: the analyser at 120 is the constraint", ok: true }, { t: "They rise to 200, since every collected sample now gets processed", bug: "notconstraint" }, { t: "They fall, because the lab gets crowded with extra samples", bug: "inversion" }] },
    hooks: [
      { id: "neck.h1", gap: 1, q: { stem: "In a flow of steps in a line, improving a step that is not the slowest usually…", options: [{ t: "changes nothing except where the queue forms", ok: true }, { t: "helps a little, in proportion to how much faster it gets", bug: "notconstraint" }, { t: "helps a lot, because every step feeds the next one", bug: "surface" }] } },
      { id: "neck.h2", gap: 7, q: { stem: "An inbox holds on average 40 unanswered emails; you answer 20 a day. How long does a typical email wait?", options: [{ t: "About 2 days", ok: true, why: "40 emails ÷ 20 a day." }, { t: "About half a day", bug: "inversion" }, { t: "Can't know without timing each email", bug: "omission" }] } },
      { id: "neck.h3", gap: 30, q: { stem: "You fixed the constraint and the queue appeared somewhere else. This means…", options: [{ t: "the constraint moved to the next slowest step", ok: true }, { t: "the fix failed and the old constraint is still there", bug: "consistency" }, { t: "the system is random, so queues appear by chance", bug: "omission" }] } },
    ],
    deeper: [
  ...
  sessions.push({
    ...
    steps: [
      { id: "q1", type: "q", kind: "predict", stage: "predict", min: 2, src: ["qex"], atoms: ["question", "judgment"], stem: "A restaurant owner must decide this week whether to open a second branch. She already knows her rent, her savings, and that her regulars love the food. Which question should she ask first?", alt: "For each question, imagine the two most different possible answers. Would she decide differently?", options: [{ t: "Will enough people there pay her prices?", ok: true, why: "In the new neighbourhood; the answer can flip the decision. It's unknown, and one answer means 'open', the other 'don't': it can flip the decision." }, { t: "Do her regulars like her food enough to follow her there?", bug: "confirm", why: "She already knows. A known answer can't change anything." }, { t: "What colour should the new sign be to attract passers-by?", bug: "irrelevant", why: "No answer to it changes whether to open." }, { t: "What is the history of the building, and why did the last tenant leave?", bug: "irrelevant", why: "Interesting, but neither answer changes the decision." }] },
      ...
      { id: "q3", type: "q", kind: "predict", stage: "predict", min: 1, atoms: ["info", "prob"], terms: ["indiff"], stem: "When is the question about demand worth the most to her?", options: [{ t: "When she's unsure, near where both choices look equally good", ok: true, why: "Near the [[indiff|point of indifference]], a small push either way flips the choice; that's where an answer pays most." }, { t: "When she's already almost sure demand is good, because then she can plan", bug: "confirm", why: "Then she opens anyway; the answer rarely changes anything." }, { t: "It's always worth the same, whatever she believes beforehand", bug: "omission", why: "Its worth depends on how close she is to changing her mind." }] },
      { id: "q4", type: "model", stage: "model", min: 3, model: "voi", src: ["howard"], title: "Slide your certainty", body: "Move her belief about demand from 'sure it's poor' to 'sure it's good'.", ask: "Where does the answer become worth the most? Why is it worth zero at both ends?" },
      {
        ...
        q: { stem: "What makes both questions good?", options: [{ t: "Some answers lead to different actions, and neither knew the answer yet", ok: true, why: "The three tests, passed." }, { t: "They are specific and concrete rather than vague and general", bug: "surface", why: "Specific but irrelevant questions exist ('What colour is the sign?'). Specificity isn't the test." }, { t: "They are about money and exams, which are the things that matter most", bug: "surface", why: "The topic is the surface." }, { t: "They sound professional, the way an expert would phrase them", bug: "pretension", why: "How a question sounds has nothing to do with its value." }] },
      },
      { id: "q6", type: "q", kind: "transfer", stage: "transfer", min: 1.5, atoms: ["question", "judgment"], stem: "You're deciding whether to spend the last week before the exam on unseen mock papers or on re-reading notes. The decisive question?", options: [{ t: "How do I score on unseen questions, and on which topics?", ok: true, why: "If unseen scores are high, re-reading adds little; if low, the failures show exactly what to re-read." }, { t: "Which method do top students in my year recommend for the last week?", bug: "authority", why: "Their answer is about them, not your current state." }, { t: "How many pages of notes are left to re-read before the exam?", bug: "irrelevant", why: "Page count doesn't tell you where marks are lost." }] },
      { id: "q7", type: "q", kind: "far", stage: "far", min: 1.5, src: ["q20"], atoms: ["info", "question"], stem: "A game: guess an animal with yes/no questions. What's the best first question?", options: [{ t: "“Is it bigger than a cat?”", ok: true, why: "It splits the possibilities roughly in half, whatever the answer: the most information per question." }, { t: "“Is it a lion, a tiger or another big cat?”", bug: "irrelevant", why: "Almost always 'no', which tells you almost nothing." }, { t: "“Does it have a common name?”", bug: "irrelevant", why: "Every answer is 'yes'." }, { t: "“Is it your favourite animal?”", bug: "irrelevant", why: "The answer doesn't narrow the animal much." }] },
      {
    ...
    hooks: [
      { id: "voi.h1", gap: 1, q: { stem: "A question is worth asking before a decision when…", options: [{ t: "you don't know it and some answer would change what you do", ok: true }, { t: "it's specific and detailed enough to get a precise answer", bug: "surface" }, { t: "an expert in the field would ask it first", bug: "authority" }] } },
      { id: "voi.h2", gap: 7, q: { stem: "When is new information worth least?", options: [{ t: "When you're already nearly certain which option is best", ok: true }, { t: "When you're unsure which way to go and the stakes are high", bug: "inversion" }, { t: "Never: information is always worth the same amount", bug: "omission" }] } },
      { id: "voi.h3", gap: 30, q: { stem: "In twenty questions, the best question…", options: [{ t: "splits the remaining possibilities about in half", ok: true }, { t: "guesses the most likely answer straight away", bug: "irrelevant" }, { t: "is the most specific question you can think of", bug: "surface" }] } },
    ],
    deeper: [
  ...
  sessions.push({
    ...
    steps: [
      ...
      { id: "w2", type: "q", kind: "predict", stage: "predict", min: 2, src: ["soho"], atoms: ["question", "falsify"], stem: "You may ask one question about the dead to test bad air against water. Which one?", alt: "Both theories predict deaths in the smelliest streets. What differs between people who breathe the same air?", options: [{ t: "Where did each of the dead get their water?", ok: true, why: "People on the same street breathe the same air but may drink from different sources: the answers split the two theories." }, { t: "Did the dead live near a bad smell, like a sewer or a cesspool?", bug: "confirm", why: "Both theories predict yes: the pumps and the smells are in the same streets." }, { t: "How many died, and how quickly after the first case?", bug: "irrelevant", why: "The count is terrible, but it can't tell the theories apart." }, { t: "What do the leading physicians of London believe causes it?", bug: "authority", why: "They believe miasma. That's the theory under test, not the evidence." }] },
      ...
      { id: "w4", type: "q", kind: "predict", stage: "predict", min: 1.5, src: ["decline"], terms: ["posthoc"], atoms: ["falsify", "causal"], stem: "After the handle was removed, new cases fell. Does that prove the pump caused the outbreak?", options: [{ t: "Not on its own: deaths were already falling before the handle came off", ok: true, why: "People had fled and the contamination may have passed. The strongest evidence was who drank from the pump and who didn't. Snow himself saw the outbreak was already declining. The story of the handle is dramatic; the comparison of drinkers and non-drinkers is the evidence." }, { t: "Yes: cases fell right after the handle was removed", bug: "posthoc", why: "They were already falling. 'After' is not 'because'." }, { t: "No: if deaths were already falling, it shows the pump was harmless", bug: "inversion", why: "It shows nothing either way; the water comparison does the work." }] },
      { id: "w5", type: "model", stage: "model", min: 3, model: "street", src: ["grand"], terms: ["natexp2"], title: "The street with two water companies", body: "In south London, two companies piped water down the **same streets**: Southwark & Vauxhall drew from the Thames below the sewage outfalls; Lambeth had moved its intake upstream in 1852.", ask: "Before you colour the houses: if bad air caused cholera, would neighbouring houses on one street differ? Then switch on the colouring." },
      {
        ...
        q: { stem: "Why are both comparisons so convincing?", options: [{ t: "Who ended up in each group had nothing to do with their risk", ok: true, why: "So the difference points at the one thing that differed. Neither the day of arrival nor the pipe was chosen by sick or healthy people: a natural experiment." }, { t: "Both involved many deaths, so chance can be ruled out", bug: "surface", why: "Big numbers help with chance, not with the filter." }, { t: "Both men were doctors whose authority carried weight", bug: "authority", why: "Their authority persuaded few people at the time; the design persuades now." }, { t: "Both are about infections, where causes are easy to see", bug: "surface", why: "The topic is the surface; the assignment is the mechanism." }] },
      },
      { id: "w7", type: "q", kind: "transfer", stage: "transfer", min: 1.5, atoms: ["question", "experiment"], stem: "After a wedding, 30 of 120 guests fall ill. What is the first thing to find out?", options: [{ t: "What each guest ate and drank, compared dish by dish", ok: true, why: "Compare the rate of illness among those who had each dish with those who didn't. It's the Broad Street question: what differs between the sick and the well who shared the same room." }, { t: "Which guests felt unwell first, and how long after the meal", bug: "irrelevant", why: "Timing helps later; alone it can't point at a cause." }, { t: "Whether the venue smelled bad or had poor ventilation", bug: "confirm", why: "Everyone shared the venue." }] },
      { id: "w8", type: "q", kind: "far", stage: "far", min: 1.5, atoms: ["question", "falsify"], stem: "A website's sign-ups dropped on Tuesday. Two theories: the new page design (shown to half the visitors) or a payment outage (hitting everyone). The decisive check?", options: [{ t: "Compare sign-ups for new-design and old-design visitors that day", ok: true, why: "Same day, so the outage hit both groups equally. The outage hit both groups; only the design differed between them. Same logic as the two water companies." }, { t: "Look at Tuesday's total and compare it with last Tuesday", bug: "irrelevant", why: "Both theories predict the drop." }, { t: "Ask the designer whether the new page could have caused it", bug: "authority", why: "Opinions don't split the theories." }] },
      {
        ...
        critique: { stem: "After two weeks, bad nights follow late screens, but late screens also follow stressful days. What now?", options: [{ t: "Stress may be a confound: split stressful days by screen use", ok: true, why: "Compare stressful days with and without late screens. Hold the third factor fixed, like the same street with different water." }, { t: "Conclude that screens cause it and ban them after midnight", bug: "confound", why: "Stress could drive both." }, { t: "Give up: with so many factors it's too complex to untangle", bug: "omission", why: "One more split usually untangles it." }] },
      },
    ],
    challenge: { stem: "Two villages share a market; only one has a new well. Diarrhoea falls in that village. The strongest extra check?", options: [{ t: "Compare new-well and old-well households in that village", ok: true, why: "Same village, same market; only the water differs." }, { t: "Ask villagers in both places whether they feel healthier", bug: "confirm" }, { t: "Count how many people live in each village and their ages", bug: "irrelevant" }] },
    hooks: [
      { id: "snow.h1", gap: 1, q: { stem: "Why was 'where did the dead drink?' a better question than 'did they live near a bad smell?'", options: [{ t: "Water differed between people sharing the same air", ok: true, why: "Smell didn't." }, { t: "Smells can't be measured, so the smell question has no answer", bug: "omission" }, { t: "Snow was an expert on water and knew where to look", bug: "authority" }] } },
      { id: "snow.h2", gap: 7, q: { stem: "Cases fell after the pump handle was removed. The flaw in calling that proof?", options: [{ t: "Cases were already falling before it", ok: true, why: "'After' isn't 'because'." }, { t: "The pump was fine, and the real source was the air", bug: "inversion" }, { t: "There is no flaw: the fall right after proves it", bug: "posthoc" }] } },
      { id: "snow.h3", gap: 30, q: { stem: "What made the Southwark & Vauxhall vs Lambeth comparison powerful?", options: [{ t: "Neighbours got different water for reasons unrelated to risk", ok: true }, { t: "It used thousands of houses, so chance was ruled out", bug: "surface" }, { t: "It was published in a book that other doctors could check", bug: "authority" }] } },
    ],
    deeper: [
  ...
  sessions.push({
    ...
    steps: [
      ...
      { id: "d4", type: "model", stage: "model", min: 3.5, model: "growth", src: ["math"], terms: ["logs", "scurve"], title: "Grow a new sense", body: "Watch the same process with ordinary eyes, then switch on the logarithmic scale.", ask: "**Find:** on the log scale, what does steady doubling look like? Then switch on the limit: where does the straight line bend?" },
      {
        ...
        q: { stem: "What do they share, beyond 'both grow fast'?", options: [{ t: "Growth in proportion to the current amount", ok: true, why: "So a constant doubling time, until a limit. The time scales differ by a factor of 50,000; the shape is the same." }, { t: "Both are technologies that improve with investment", bug: "surface", why: "One is biology." }, { t: "Both grow by the same amount each step, only at different speeds", bug: "linear", why: "That's linear growth. They grow by the same **factor**." }, { t: "Both will never stop growing as long as there is demand", bug: "omission", why: "The flask shows the limit; chips hit physical limits too." }] },
      },
      { id: "d6", type: "q", kind: "transfer", stage: "transfer", min: 1.5, src: ["math"], atoms: ["scale"], stem: "Savings grow 10% a year. About how long to go from 1,000 to 8,000?", options: [{ t: "About 21 years", ok: true, why: "Three doublings of about 7 years: 1,000 → 2,000 → 4,000 → 8,000. 1,000 → 2,000 → 4,000 → 8,000." }, { t: "About 70 years", bug: "linear", why: "That's adding 100 a year." }, { t: "About 8 years", bug: "surface", why: "That's one doubling and a bit." }] },
      { id: "d7", type: "q", kind: "far", stage: "far", min: 1.5, atoms: ["scale", "predict"], stem: "A rumour: the number who know it doubles every day, starting with one person. About how long until a thousand know?", options: [{ t: "About 10 days", ok: true, why: "2¹⁰ = 1,024. And a million only 10 days after that." }, { t: "About 500 days", bug: "linear", why: "That's one or two new people a day." }, { t: "About 100 days", bug: "linear", why: "Still straight-line thinking." }] },
      {
        ...
        critique: { stem: "The log plot of your users is a straight line, then bends downward. What does that mean?", options: [{ t: "Growth is slowing: you're nearing a limit", ok: true, why: "Or the loop that drove it weakened. A downward bend on log scale = the doubling time is getting longer." }, { t: "You're losing users, and the total is falling", bug: "inversion", why: "A bend down on a log plot can still be growth, just slower." }, { t: "Nothing: it's still exponential, just noisier", bug: "linear", why: "A straight line is exponential; a bend is not." }] },
      },
    ],
  ...
  sessions.push({
    ...
    steps: [
      { id: "t1", type: "q", kind: "predict", stage: "predict", min: 1.5, src: ["pairs"], atoms: ["taste", "compress"], stem: "Which sentence is better for a hospital report?", panels: [{ title: "Left", body: "It is important to note that there has been a significant increase in the number of patients who are experiencing delays in being seen at the present time." }, { title: "Right", body: "More patients now wait longer to be seen." }], options: [{ t: "Right: the same claim, every word working", ok: true, why: "Left spends 20 extra words (28 against 8) on nothing the reader needs ('it is important to note that', 'at the present time')." }, { t: "Left: it sounds more professional for a report", bug: "pretension", why: "Sounding official isn't information. Readers skim padded sentences and miss the point." }, { t: "Left: longer means more complete and careful", bug: "surface", why: "It contains no fact that Right lacks." }, { t: "No real difference once you read both", bug: "omission", why: "Time the reading of each: the difference is real." }] },
      ...
      { id: "t4", type: "q", kind: "predict", stage: "predict", min: 1.5, src: ["cleveland", "tufte"], atoms: ["taste", "represent"], stem: "Same four numbers, two charts. Which lets a reader compare the wards faster and more accurately?", panels: [{ title: "Left", svg: "chartBad" }, { title: "Right", svg: "chartGood" }], options: [{ t: "Right: sorted bars on one scale, values written on", ok: true, why: "People judge positions on a common scale accurately; 3D shading, a colour legend and gridlines make them work harder for nothing." }, { t: "Left: 3D bars look more serious and polished", bug: "pretension", why: "3D distorts lengths and adds nothing." }, { t: "Left: more colours carry more information", bug: "surface", why: "The colours just repeat the ward names, which you must decode through the legend." }, { t: "They're equal: the data is the same", bug: "omission", why: "Same data, different reading time and error rate." }] },
      {
        ...
        q: { stem: "What single fault do they share?", options: [{ t: "They make the reader process material that carries nothing", ok: true, why: "Padding in words, decoration in ink: the same tax." }, { t: "They're both too long for a reader to finish", bug: "surface", why: "Length isn't the fault; empty length is." }, { t: "They're both unprofessional and badly formatted", bug: "pretension", why: "Both were probably made to look professional. That's the problem." }] },
      },
      { id: "t6", type: "q", kind: "transfer", stage: "transfer", min: 1.5, src: ["pairs"], atoms: ["taste", "judgment"], stem: "A medicine label will be read by a tired parent at 3 am. Which label is better?", panels: [{ title: "Left", svg: "labelBad" }, { title: "Right", svg: "labelGood" }], options: [{ t: "Right: one line is clearly first, so the eye knows where to start", ok: true, why: "When everything is shouted, nothing is. Hierarchy is information." }, { t: "Left: bold capitals are more serious for a warning", bug: "pretension", why: "All-caps bold everywhere removes the one signal that matters most: which line to read first." }, { t: "Left: it carries the same information", bug: "surface", why: "The information matches; how fast a tired reader finds the dose limit does not." }] },
      { id: "t7", type: "q", kind: "far", stage: "far", min: 1.5, src: ["gopen"], atoms: ["taste", "compress"], stem: "The title of a slide showing Semmelweis's 1847 data. Which is better?", options: [{ t: "\"Hand-washing cut deaths from 18% to 2% within two months\"", ok: true, why: "The title is the finding. A viewer who reads only titles still gets the talk." }, { t: "\"Results\"", bug: "omission", why: "It names the slide instead of saying anything." }, { t: "\"A comprehensive overview of the mortality outcomes observed\"", bug: "pretension", why: "Long, official, and still says nothing." }] },
      {
        ...
        critique: { stem: "A colleague says your short version sounds too casual for a presentation. The best answer?", options: [{ t: "It keeps every clinical fact and is faster to follow", ok: true, why: "Formality comes from precision, not padding. Criteria you can test beat a feeling of formality." }, { t: "Add the padding back to be safe in front of seniors", bug: "pretension", why: "Padding costs the listener attention." }, { t: "Remove the duration too, to make it even shorter", bug: "omission", why: "Duration is clinical information: never cut conditions." }] },
      },
    ],
    challenge: { stem: "Which is the better sentence? A: \"The committee made a decision to implement a reduction of costs.\" B: \"The committee decided to cut costs.\"", options: [{ t: "B: same meaning, fewer words", ok: true, why: "Verbs instead of nominalisations." }, { t: "A: more formal and more precise", bug: "pretension" }, { t: "Equal: both say the same thing", bug: "omission" }] },
    hooks: [
      { id: "taste.h1", gap: 1, q: { stem: "What makes a sentence 'padded'?", options: [{ t: "Words that could go without losing anything the reader needs", ok: true }, { t: "Being longer than about ten words in a single sentence", bug: "surface" }, { t: "Using technical terms that the reader might not know", bug: "pretension" }] } },
      { id: "taste.h2", gap: 7, q: { stem: "Why are sorted bars with values usually better than a 3D pie chart?", options: [{ t: "People compare positions on one scale more accurately than angles", ok: true, why: "Or 3D shapes." }, { t: "Bars look more modern and professional on slides", bug: "surface" }, { t: "Pie charts are always wrong and should never be used", bug: "omission" }] } },
  ...
  sessions.push({
    ...
    steps: [
      ...
      { id: "x2", type: "q", kind: "alien", stage: "far", min: 2, src: ["bevan2"], atoms: ["orient", "measure"], stem: "Stays got 20% shorter and readmissions rose 30%. The most likely mechanism?", options: [{ t: "A stay target was met by sending patients home too early", ok: true, why: "The number improved; recovery didn't. A measure became the target (session 5)." }, { t: "Patients are getting older and sicker every year", bug: "omission", why: "Possible, but it doesn't explain why the rise came with the new stay target." }, { t: "Doctors got better at treatment, so stays got shorter", bug: "inversion", why: "Better treatment would lower readmissions." }, { t: "Bad luck in one unusual year", bug: "omission", why: "A 30% rise alongside a 20% cut is a pattern, not luck." }] },
      { id: "x3", type: "q", kind: "alien", stage: "far", min: 2, atoms: ["orient", "causal"], stem: "Survey scores rose while complaints rose. Surveys are handed out at the discharge desk. What explains both?", options: [{ t: "A filter: surveys reach the relieved, complaints come from the rest", ok: true, why: "Surveys reach people leaving, not those readmitted or those who left angry; complaints come from exactly those. Who gets asked shapes the answer (session 2)." }, { t: "People lie on surveys to please the staff who hand them out", bug: "moral", why: "No lying needed: the filter alone produces the gap." }, { t: "Patients became more demanding, so they complain about more things", bug: "omission", why: "It doesn't explain why surveys improved at the same time." }] },
      { id: "x4", type: "q", kind: "alien", stage: "far", min: 2, atoms: ["orient", "systems"], stem: "When wards overflow, managers add beds in next spring's budget; the beds open in autumn, when winter demand is ending; then they're cut again. What will the bed count do?", options: [{ t: "Swing up and down, always a season late", ok: true, why: "A correction loop with a delay. The shower from session 4, with beds." }, { t: "Settle at the right number after a year or two", bug: "nodelay", why: "Only if the delay were short compared with the swings in demand." }, { t: "Grow forever, since demand keeps rising", bug: "inversion", why: "Cuts follow every rise." }] },
      { id: "x5", type: "q", kind: "alien", stage: "far", min: 2, atoms: ["orient", "prob"], stem: "The sepsis alarm flags 10 of every 100 patients. About 2 of every 100 have sepsis, and the alarm catches 90% of them. Why do nurses ignore it?", options: [{ t: "Of about 10 alarms, only about 2 are real", ok: true, why: "Most alarms are false, so ignoring them is learned. Count it (session 3): 1.8 true alarms among 10. A rare condition makes a busy alarm mostly wrong." }, { t: "Nurses are careless and overworked on busy wards", bug: "moral", why: "The numbers teach them to ignore it; carelessness isn't needed." }, { t: "The alarm catches too few of the septic patients", bug: "inverse", why: "It catches 90%. The problem is how many healthy patients it flags." }] },
      ...
      { id: "x7", type: "q", kind: "alien", stage: "far", min: 2, atoms: ["minimal", "judgment"], stem: "Emergency patients are seen within 4 hours but wait on trolleys for beds. Beds are full because patients ready to leave wait for community care, which can take only 10 a day. Which single change helps most?", options: [{ t: "Raise community-care capacity", ok: true, why: "It's the constraint that fills the beds, forces early discharges and blocks the emergency department. Three boxes (session 7): emergency → ward → community. The last box sets the pace." }, { t: "Hire more emergency staff to see patients even faster", bug: "notconstraint", why: "They're not the constraint: patients already wait after being seen." }, { t: "A stricter 4-hour target with penalties for missing it", bug: "proxy", why: "More pressure on a number that is already met, while the constraint is elsewhere." }, { t: "More surveys to find out what patients want", bug: "selection", why: "More filtered feedback." }] },
      {
        ...
        q: { stem: "What's the deep sameness?", options: [{ t: "A reward on a number cheaper to move than the reality", ok: true, why: "A century apart, one mechanism." }, { t: "Both were run by governments that ignored the people affected", bug: "surface", why: "Private firms do it too." }, { t: "Both involved living things: animals and patients", bug: "surface", why: "Surface." }] },
      },
      { id: "x9", type: "q", kind: "transfer", stage: "transfer", min: 1.5, atoms: ["orient", "synth"], stem: "A school's pass rate jumps after weaker students are advised not to sit the exam. Which two mechanisms?", options: [{ t: "A rewarded number and a filter on who is counted", ok: true, why: "Goodhart plus selection: the rate went up because the denominator changed." }, { t: "Better teaching and exponential growth in student effort", bug: "omission", why: "Nothing in the story says teaching changed." }, { t: "A delay loop between advice and results", bug: "omission", why: "No correction or delay is described." }] },
      {
        ...
        critique: { stem: "A board member says: 'Five symptoms, so we need five separate projects.' Your reply?", options: [{ t: "Two symptoms share causes: raise the constraint and drop the stay target first", ok: true, why: "Four mechanisms explain all five. The minimum sufficient model tells you where one change does the most." }, { t: "Agree: one project per symptom, each with its own team", bug: "notconstraint", why: "That spreads effort over symptoms that share causes." }, { t: "Start with the surveys, since patients' views matter most", bug: "selection", why: "Surveys are the filtered part, not a cause." }] },
      },
    ],
    challenge: { stem: "A startup's users grew 50% while revenue stayed flat and support tickets tripled; the team is rewarded for sign-ups. First suspicion?", options: [{ t: "Sign-ups became the target, inflated by low-intent users", ok: true, why: "They also load support." }, { t: "The product got worse as it grew more complicated", bug: "omission" }, { t: "Exponential growth, which always strains support", bug: "surface" }] },
    hooks: [
      { id: "boss.h1", gap: 1, q: { stem: "Survey scores up, complaints up. First question?", options: [{ t: "Who gets the survey and who doesn't?", ok: true }, { t: "Are patients lying on the survey forms?", bug: "moral" }, { t: "How many surveys were sent out this year?", bug: "irrelevant" }] } },
      { id: "boss.h2", gap: 7, q: { stem: "A rare condition plus an alarm that flags many people leads to…", options: [{ t: "mostly false alarms that people learn to ignore", ok: true }, { t: "mostly true alarms, since the condition is rare", bug: "inverse" }, { t: "almost no alarms at all", bug: "omission" }] } },
      { id: "boss.h3", gap: 30, q: { stem: "Five symptoms. Before five projects, look for…", options: [{ t: "the fewest mechanisms that explain them all", ok: true }, { t: "the loudest symptom, since it hurts most", bug: "surface" }, { t: "who to blame for letting it happen", bug: "moral" }] } },
    ],
    deeper: [
